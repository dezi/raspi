...
#define VF_LOGO_VERSION "1.0.0.2011.03.30"
...
static av_cold int init(AVFilterContext *ctx, const char *args, void *opaque)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "vf_logo: init\n");
    av_log(NULL, AV_LOG_INFO, "vf_logo: version %s\n", VF_LOGO_VERSION);

    logo->x = logo->y = logo->w = logo->h = 0;
	...
	{			
		...
		av_log(NULL, AV_LOG_INFO, "vf_logo: RGB=(%d,%d,%d) x=%d y=%d file=%s\n",
    ...
    else 
	{
		...
		if (num_fields == 3) 
		{
			...
			av_log(NULL, AV_LOG_INFO, "vf_logo: x=%d y=%d file=%s\n",
...
static int load_logo_create_frames(AVFilterContext *ctx)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "vf_logo: load_logo_create_frames\n");
	...
	av_log(NULL, AV_LOG_DEBUG, "vf_logo: avcodec_open\n");
	...
	av_log(NULL, AV_LOG_DEBUG, "vf_logo: logo size is %dx%d pix-fmt:%s\n", logo->w, logo->h, av_get_pix_fmt_name(logo->format));

	av_log(NULL, AV_LOG_DEBUG, "vf_logo: linesizes [0]=%d [1]=%d [2]=%d [3]=%d\n",
	...
	av_log(NULL, AV_LOG_DEBUG, "vf_logo: av_read_frame\n");
	
	frameFinished = 0;

	while (av_read_frame(logo->pFormatCtx, &logo->packet) >= 0)
	{		
		av_log(NULL, AV_LOG_DEBUG, "vf_logo: got a frame\n");
		...
		if (logo->packet.stream_index == logo->videoStream)
		{
			...
			av_log(NULL, AV_LOG_DEBUG, "vf_logo: avcodec_decode_video\n");
		...
		if (frameFinished)
		{
			av_log(NULL, AV_LOG_DEBUG, "vf_logo: got a finished frame, fine!!\n");
			
			av_log(NULL, AV_LOG_DEBUG, "vf_logo: avcodec decoded logo image to pix_fmt=%s\n", av_get_pix_fmt_name(logo->pCodecCtx->pix_fmt));
	...
	else
	{
		...
		av_log(NULL, AV_LOG_DEBUG, "vf_logo: transform logo image from pix_fmt=%s to RGBA\n", 
			av_get_pix_fmt_name(logo->pCodecCtx->pix_fmt));
    
		if (logo->pCodecCtx->pix_fmt == PIX_FMT_RGB24)
		{    
			av_log(NULL, AV_LOG_DEBUG, " vf_logo: image of pix_fmt=%s has no alpha path!\n", 
				av_get_pix_fmt_name(logo->pCodecCtx->pix_fmt));
		}
		...
		av_log(NULL, AV_LOG_DEBUG, "vf_logo: RGBA linesizes [0]=%d [1]=%d [2]=%d [3]=%d\n",
	...
	/*
	av_log(NULL, AV_LOG_DEBUG, "vf_logo: alpha_transparency of frame is:%d\n",
	...
	*/
	...
	{  
		...
		for (int i = 0; i < logo->h; i++)
		{
			...
			{
				//av_log(NULL, AV_LOG_DEBUG, "vf_logo: image (%3d,%3d) R=%3d G=%3d B=%3d A=%3d\n",i,j,pix->R,pix->G,pix->B,pix->A);
...
static int query_formats(AVFilterContext *ctx)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "vf_logo: query_formats\n");
...
static int config_input_main(AVFilterLink *inlink)
{
	...
	av_log(NULL, AV_LOG_INFO, "vf_logo: overlay logo of size (%d,%d) at (%d,%d) w=%d h=%d onto video of size (%d,%d) pix-fmt=%s\n",
				logo->w,       logo->h,
...
static void draw_slice_box(AVFilterLink *inlink, int y0, int h, int slice_dir)
{
    LogoContext 	  *logo   = inlink->dst->priv;
	...
	for (int y = FFMAX(yb, y0); y < (y0 + h) && y < (yb + logo->app_h); y++) 
	{
...
static void draw_slice_logo(AVFilterLink *inlink, int y0, int h, int slice_dir)
{
    LogoContext 	  *logo   = inlink->dst->priv;
	AVFilterBufferRef *picref = inlink->cur_buf;
...
{
	av_log(inlink->dst, AV_LOG_DEBUG, "draw_slice y0=%d h=%d slice_dir=%d\n", y0, h, slice_dir);
		
	draw_slice_logo(inlink, y0, h, slice_dir);
	
	//draw_slice_box(inlink, y0, h, slice_dir);
...
{
	avfilter_start_frame(link->dst->outputs[0],avfilter_ref_buffer(picref, ~0));
